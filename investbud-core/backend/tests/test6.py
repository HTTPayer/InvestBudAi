...
def get_heurist_search(session, nansen_data):
    """
    ...
    token_symbols = [t.get('token_symbol') for t in tokens if t.get('token_symbol')]
    sectors = list(set([sector for t in tokens for sector in t.get('token_sectors', [])]))
...
def run_demo():
    ...
    try:
        ...
        nansen_data = get_nansen_data(session)

        # Step 2: Get Heurist search results
        heurist_data = get_heurist_search(session, nansen_data)

        # Step 3: Summarize with LLM
        summary = summarize_with_llm(session, nansen_data, heurist_data)
        ...
        # Extract and display original text
        if isinstance(summary, dict):
            original = summary.get('original') or summary.get('response') or summary
        else:
            original = summary

        print("\n--- Original (English) ---")
        original_text = extract_text(original)
        truncated = original_text[:500]
        print(truncated)
        ...
        timestamp = datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
        original_path = output_dir / f"demo06_original_{timestamp}.md"
        ...
        print("\n[Step 5] Saving combined data for website")
        print("-" * 70)
        ...
        print(f"✓ Website data saved to: {data_path}")
